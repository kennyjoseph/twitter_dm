...
urlStart2  = r"\b(?:[A-Za-z\d-])+(?:\.[A-Za-z0-9]+){0,3}\." + regex_or(commonTLDs, ccTLDs) + "(?:\."+ccTLDs+")?(?=\W|$)"
urlBody    = r"(?:[^\.\s<>][^\s<>]*?)?"
...
EdgePunctLeft  = regex.compile(offEdge + "("+edgePunct+"+)("+notEdgePunct+")", regex.UNICODE)
EdgePunctRight = regex.compile("("+notEdgePunct+")("+edgePunct+"+)" + offEdge, regex.UNICODE)
