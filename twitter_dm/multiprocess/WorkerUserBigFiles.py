...
class BigFileUserDataWorker(multiprocessing.Process):
    def __init__(self,
        ...
        self.out_sinceid_queue = out_sinceid_queue
        self.queue = queue
        ...
        self.output_file = bz2.open(os.path.join(output_dir,"json", self.id+".json.bz2"), "wt")
        ...
        while True:
            ...
            try:
                ...
                for tweet in tweets:
                    ...
                    self.output_file.write(json.dumps(tweet) + '\n')

                if len(tweets):
                    # write out the since tweet id file
                    self.sinceid_output_file.write('\t'.join([str(user_identifier), str(tweets[0]['id'])]) + '\n')
                    self.out_sinceid_queue.put('\t'.join([str(user_identifier), str(tweets[0]['id'])]) + '\n')
                else:
                    self.sinceid_output_file.write('\t'.join([str(user_identifier), str(since_tweet_id)]) + '\n')
                    self.out_sinceid_queue.put('\t'.join([str(user_identifier), str(since_tweet_id)]) + '\n')
                ...
                print("*** print_tb:")
                traceback.print_tb(exc_traceback, limit=300, file=sys.stdout)
