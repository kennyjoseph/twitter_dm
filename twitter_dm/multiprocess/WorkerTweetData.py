...
class TweetDataWorker(multiprocessing.Process):
    def __init__(self, queue, api_hook, conn_number, out_dir, to_pickle=False, gets_user_id=True):
        ...
        self.tweet_output_file_name = self.out_dir+"/"+str(conn_number)+".json"
        self.tweet_id_output_file_name = self.out_dir+"/"+str(conn_number)+".txt"

    def run(self):
        print('Worker started')
        # do some initialization here

        tweet_output_file = open(self.tweet_output_file_name, "w")
        tweet_id_output_file = open(self.tweet_id_output_file_name, "w")
        ...
        while True:
            ...
            for tw in tweet_data:
                try:
                    good += 1
                    tweet_output_file.write(json.dumps(tw)+"\n")
                    tweet_id_output_file.write(str(tw['id']) + "\tg\n")
                    data.remove(tw['id'])
            ...
            for failed_tweet in data:
                try:
                    tweet_id_output_file.write(str(failed_tweet) + "\tf\n")
                except KeyboardInterrupt as e:
                    print(e)
                    break
                except:
